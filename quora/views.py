...
def index(request):
    ...
    if query:
        questions = Question.objects.filter(text__icontains=query).order_by('-date_added')
    else:
    ...
    return render(request, 'quora/index.html', {'questions':questions})
...
def tag(request, tag_id):
    ...
    questions = tag.question_set.all().order_by('-date_added')
    paginator = Paginator(questions, 5)
    page = request.GET.get('pg', 1)
    questions = paginator.get_page(page)

    return render(request, 'quora/tag.html', {'tag':tag, 'questions':questions})
